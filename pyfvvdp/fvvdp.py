            ...
            for bb in range(self.lpyr.get_band_count()-1):
                ...
                if N_nCSF[bb][cc] is None:
                    ...
                    if self.foveated:   # Fixation, parafoveal sensitivity
                        ...
                        view_dir = self.display_geometry.pix2view_direction( torch.tensor((w_band, h_band)), xx, yy )
                        view_dir_gaze = self.display_geometry.pix2view_direction( torch.tensor((w_frame, h_frame)), 
                        ...
                        ecc = torch.sqrt(torch.sum((view_dir-view_dir_gaze)**2, dim=0, keepdim=False ))

                        # The same shape as bands
                        ecc = ecc.reshape( [1, 1, ecc.shape[-2], ecc.shape[-1]] )

                        res_mag = self.display_geometry.get_resolution_magnification(view_dir)
